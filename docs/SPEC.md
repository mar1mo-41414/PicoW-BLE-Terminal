...
| `picow-ota/` (framework) | ステージング / メタデータ / apply / confirm | プロトコル安定 |
| `bootloader/` | 独立ブートローダ (別 CMake target) | 別プロセス感覚 |
| `picow-ota/linker/` | 3 種類の `memmap_*.ld` | フラッシュ拡張時のみ |
| `include/` | 公開ヘッダ・SDK 設定ヘッダ | `btstack_config.h`, `lwipopts.h` (metadata は picow-ota 側) |
...
- `pico_ota_bootloader` — ブートローダ (0x10000000 起点)
...
2. `PICO_OTA_SLOT=0` または `=1` を `target_compile_definitions` で注入
...
定数はすべて `picow-ota/include/pico_ota/metadata.h` に集約:
...
### ブートローダの判定ロジック(`picow-ota/bootloader/main.c`)
...
- `main()` → `ota_init()` がメタデータを読み `PICO_OTA_SLOT` と照合、
  不整合なら警告ログ
...
`picow-ota/include/pico_ota/metadata.h`:
...
- `config.c` / `ota.c` / bootloader すべて `pico_ota_crc32()` を共有(picow-ota/src/crc32.c)
...
- staging slot は「自分でない方」(compile time の `PICO_OTA_SLOT` から決定)
...
- `picow-ota/include/pico_ota/sha256.h` — 自前 SHA-256
...
- `picow-ota/include/pico_ota/ota.h` — OTA API
- `picow-ota/include/pico_ota/metadata.h` — メタデータ struct + パーティション定数
